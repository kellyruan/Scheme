...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('length of {0} and {1} do not match'.format(formals, vals))
        else:
            while formals:
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('incorrect number of arguments were passed')
...
def do_or_form(expressions, env):
    ...
    if expressions == nil:
        ...
        if scheme_truep(first_eval):
            return first_eval
        if expressions.second == nil and scheme_falsep(first_eval):
            return False
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        syms = Pair(bindings.first.first, syms)
        exps = Pair(scheme_eval(bindings.first.second.first, env), exps)
        bindings = bindings.second
    check_formals(syms)
    return env.make_child_frame(syms, exps)
